...
(function(){
     ...
     * @param pageSize	   = 한 페이지에 나타낼 데이터 수
     * @param callback     = callback 함수
     */
    window.paging = window.customPaging = function(target, totalDataCnt, pagingData, currentPage, pageSize, callback){
    	pagingObj.dataPerPage = pageSize;
...
//        curPageId = currentPage>1?Math.round((currentPage/pagingObj.dataPerPage))+1:currentPage;
        ...
        if(pagingObj.prev > 0) pagingHtml += '<li><a href="javascript:void(0)" class="prev" id="prev">이전</a></li>';
...
//        $(target).addClass('num');  // TODO : 각 프로젝트 스타일에 맞게 수정 및 삭제 필요
        ...
        // 현재 페이지 표시
        $(target + ' li#'+curPageId).addClass('active');  // TODO : 각 프로젝트 스타일에 맞게 수정 및 삭제 필요
